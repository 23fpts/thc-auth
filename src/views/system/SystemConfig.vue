        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="参数配置" prop="configLike">
              <el-input v-model="configQueryform.configLike" placeholder="请输入参数名称或参数编码" />
            </el-form-item>
          </el-col>
          <el-col :span="6" :offset="6">
            <el-form-item>
              <el-button type="primary" size="small" @click="submitQueryForm()" icon="el-icon-search">
                查询</el-button>
              <el-button type="primary" size="small" plain @click="resetQueryForm()" icon="el-icon-refresh">
                重置</el-button>
            </el-form-item>
          </el-col>
        </el-row>
...
<script>
import { getConfigs, updateConfig, addConfig, deleteConfig } from '@/api/system'
  ...
  methods: {
    handleRefresh() {
      this.$store.dispatch('refreshConfig').then((res) => {
        this.$message({
          message: '刷新配置参数到应用中成功！',
          type: 'success',
        })
      })
    },
    getData() {
      getConfigs(this.configQueryform.configLike).then((res) => {
        this.setData(res)
      })
    },
    updateData() {
      updateConfig(this.dialogForm).then((res) => {
        this.handleRefresh()
    ...
    deleteData(row) {
      this.$confirm('确定删除参数:[' + row.paramName + ']?').then((_) => {
        deleteConfig(row.id)
          .then((res) => {
            this.handleRefresh()
