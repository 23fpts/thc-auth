<template>
  <el-select ref="tsSelect" :value="selectedTitle" :clearable="clearable" :disabled="disabled" @clear="clearSelected" @change="validateSelectTree" placeholder="直接上级组织" style="width: 100%">
    <el-option :value="selectedTitle" :label="selectedTitle">
      <el-tree id="tree-option" ref="selectTree" :accordion="accordion" :data="elTreeData" :props="elTreeProps" :node-key="elTreeProps.value" :default-expanded-keys="defaultExpandedKey" @node-click="handleNodeClick">
      </el-tree>
    </el-option>
  </el-select>
</template>
  ...
  methods: {
    ...
    initHandle() {
      if (this.selectedId && this.elTreeData.length > 0) {
        ...
        this.$nextTick(() => {
          // 待组件渲染完成，执行代码
          // nextTick表示等待组件渲染完成之后再执行其中的代码，如果不这样做。组件没渲染完成，this.$refs.selectTree可能为空。
